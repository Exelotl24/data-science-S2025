...
-   Which rows did you pick?
    -   All rows without NA values on the latest date of the data set were chosen for the means and SDs and rows where population = 0
-   Why?
    -   NA values would have interefered with the calculations, and 0 population would be meaningless when calculating cases and deaths per 100k. Showing all mean cases of every day also is not a very meaningful quantity to compute
...
-   Loving county had the most cases by quite a bit especially for having a very small population of 102. McMullen also had the most deaths, however not by as large a margin as Loving, and it also has a rather small population of 662.
-   When did these "largest values" occur?
    -   The largest values for cases all happened in may of 2022 and the value for deaths occurred in february of 2022.
...
-   It seems that generally counties with less populations have more deaths per 100k in Virginia, which makes sense as the percentage of someone dying due to covid has a greater impact on the population than a large city. However there is an exception where one county (guessing by the color and population most likely Alexandria or somewhere else in Northern VA) where there is a significant number of deaths.

-   In the second graph, focusing on Richmond, VA, it seems that generally, deaths increased over time, however there was a point in the second half of 2021 where deaths decreased, which could indicate stronger mandates or restrictions (if i recall correctly fall 2021 was a second wave of a new strain so it would make sense that there were more mandates)
