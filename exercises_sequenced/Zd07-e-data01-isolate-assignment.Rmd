...
df_q3 <- 
  flights %>% 
  filter(near(arr_time, sched_arr_time, tol = 10)
  )
...
```{r q4-task}
flights %>% 
  filter(dest == 'LAX') # Uncomment and run to see error
```
...
**Observations** - Was this dataset assembled to study flights *out of* NYC, or *into* NYC? How do you know? - Out of NYC because when the destination is in NY, the flight is leaving from NY, and when the origin is from NY, the destinations are across the country
