...
**Observations**: - Write your observations here! - Generally there is not a meaningful difference between the mean velocity values as distinctness changes. The difference values between distinctness of 1-3 and 2-3 are quite similar at 53.7 and 50.8 respectively.

Why might your table differ from Michelson's? - It looks like there is a difference with the significant figures in Michelson's data, my table does not have a limit on significant figures and Michelson's has 5.
...
**Observations**: - Is Michelson's estimate of the error (his uncertainty) greater or less than the true error? - Michelson's estimation of error is less than the value of the true error as he estimated it to be 51 and it is -151.542 - Make a quantitative comparison between Michelson's uncertainty and his error. - The true error is 100.542 greater than Michelson's uncertainty
...
**Observations**: Similarities - The mean for both the simulated and real measurements are the same, so are the positive and negative error values. Certain means for each day are similar between the graphs, such as on Jun 16.

Differences - The real mean values for each day tend to have more "dramatic" spikes (ones that fall outside of the control limits), as this graph has three spikes of that nature, while the simulated graph only has two. The simulated graph also tends to have more individual data points that lay close to the median line than the real data.
...
-   The velocity in a vacuum overall slight trends upwards as temperature increases, however there is much variation in the graph. This is not a possible explanation for the difference between LIGHTSPEED_VACUUM and Michelson's data as the data varies greatly.
...
-   This graph shows that there is overall a slight upward trend between velocity in a vacuum and distinctness, however the difference between 2 and 3 distinctness is significantly less than the difference from 1 to 2. This could potentially lead to fluctuations in Michelson's data, however I was interested to see what effect distinctness has compared to the velocity per day, which I did below.
...
-   In the graph above, I looked at the different levels of distinctness versus the simulated and real velocities. It does not appear that distinctness has a distinct effect on velocity in the real or simulated velocity values. In the real measurements, lower mean velocity values tend to have a distinctness of 1, however higher velocity values do not necessarily correlate to a distinctness of 3.
